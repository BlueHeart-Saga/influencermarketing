          ...
          {POPULAR_CATEGORIES.map((category) => (
            ...
            >
              <View style={styles.categoryIconContainer}>
                <Icon name={category.icon} size={22} color={PRIMARY_BLUE} />
              </View>
      ...
      <View style={styles.tabBar}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            if (searchResults.length === 0) {
              Alert.alert('Information', 'Please search for an influencer first to view the results.');
            } else {
              setActiveTab('results');
            }
          }}
        >
          <Icon
            ...
            size={20}
            color={activeTab === 'results' ? PRIMARY_BLUE : (searchResults.length === 0 ? '#cbd5e1' : '#64748b')}
          />
          <Text style={[
            styles.tabText,
            activeTab === 'results' && styles.activeTabText,
            searchResults.length === 0 && { color: '#94a3b8' }
          ]}>
            Results
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          onPress={() => {
            if (!selectedInfluencer) {
              Alert.alert('Information', 'Please select an influencer from the results to view comprehensive details.');
            } else {
              setActiveTab('details');
            }
          }}
        >
          <Icon
            ...
            size={20}
            color={activeTab === 'details' ? PRIMARY_BLUE : (!selectedInfluencer ? '#cbd5e1' : '#64748b')}
          />
          <Text style={[
            styles.tabText,
            activeTab === 'details' && styles.activeTabText,
            !selectedInfluencer && { color: '#94a3b8' }
          ]}>
            Details
          </Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  categoryCard: {
    width: (width - 32 - 24) / 3, // padding: 16*2=32, gap: 12*2=24
    backgroundColor: '#fff',
    paddingVertical: 20,
    paddingHorizontal: 8,
    borderRadius: 16,
