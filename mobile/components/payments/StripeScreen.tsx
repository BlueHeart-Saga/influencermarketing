...
import { initStripe, useStripe, CardField, PaymentSheet } from '@stripe/stripe-react-native';
...
const StatusBadge = ({ status, type = 'payment' }: { status: string; type?: 'payment' | 'application' }) => {
  const config = type === 'payment'
    ? stripeAPI.getPaymentStatusConfig(status)
    : stripeAPI.getApplicationStatusConfig(status);
...
const MediaFilesModal = ({ visible, onClose, mediaFiles, application }: {
  ...
  const handleDownload = async (media: MediaFile) => {
    try {
      await stripeAPI.downloadMediaFile(media.file_id, media.filename);
    } catch (error) {
  ...
  const renderMediaItem = ({ item }: { item: MediaFile }) => (
    ...
    >
      <View style={styles.mediaIcon}>
        <Feather name={getMediaTypeIcon(item.media_type) as any} size={24} color="#2563eb" />
      </View>
...
const PaymentSheetModal = ({
  ...
  const handlePresentPaymentSheet = async () => {
    try {
      ...
      if (error) {
        console.error('Payment sheet present error:', error);
        onError(error.message);
      } else {
        onSuccess();
      }
  ...
  return (
    ...
    >
      <View style={styles.modalOverlay}>
        <View style={styles.paymentSheetModal}>
          <LinearGradient
            colors={['#2563eb', '#1d4ed8']}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
            style={styles.paymentSheetHeader}
          >
            <Text style={styles.paymentSheetTitle}>Pay with Stripe</Text>
            <TouchableOpacity onPress={onClose}>
              <Feather name="x" size={24} color="#fff" />
            </TouchableOpacity>
          </LinearGradient>
          ...
          <View style={styles.paymentSheetBody}>
              ...
              <View style={styles.paymentSummary}>
                ...
                <View style={styles.paymentSummaryRow}>
                  ...
                  <Text style={styles.paymentSummaryAmount}>
                    {formatCurrency(application.budget || application.campaign_budget || 0, application.currency || 'USD')}
                  </Text>
                </View>
              </View>
            )}
            ...
            {loading ? (
              <View style={styles.loadingContainer}>
                ...
                <Text style={styles.loadingText}>Loading payment sheet...</Text>
              </View>
            ) : (
              <TouchableOpacity
...
const StripeScreen = () => {
  ...
  const params = useLocalSearchParams<{ campaign: string; influencer: string }>();
  const initialPaymentRef = useRef(false);
  ...
  useEffect(() => {
    if (params.campaign && params.influencer && applications.length > 0 && !initialPaymentRef.current) {
      const targetApp = applications.find(
        app => app.campaign_id === params.campaign && app.influencer_id === params.influencer
      );
      if (targetApp) {
        handleOpenPaymentDialog(targetApp);
        initialPaymentRef.current = true;
      }
    }
  }, [params.campaign, params.influencer, applications]);
  ...
  const loadMediaFiles = async (application: Application) => {
    ...
    try {
      ...
      try {
        const response = await stripeAPI.getCampaignMediaFiles(
  ...
  const handleOpenPaymentDialog = async (application: Application) => {
    ...
    try {
      ...
      const paymentData = {
        ...
        currency: (application.currency || application.campaign_currency || 'usd').toLowerCase(),
      };
      ...
      Alert.alert('Error', err.detail || err.message || 'Failed to initialize payment');
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handlePaymentSuccess = async () => {
    ...
    Alert.alert(
      'Payment Successful!',
      `Payment of ${formatCurrency(selectedApplication?.budget || 0, selectedApplication?.currency || 'USD')} completed successfully.`,
      [{ text: 'OK' }]
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        >
          <View>
            <Text style={styles.headerTitle}>Stripe Payment Management</Text>
        ...
        <View style={styles.tabsContainer}>
          ...
          >
            <Feather
              name="credit-card"
              size={18}
              color={activeTab === 'pending' ? '#2563eb' : '#999'}
            />
